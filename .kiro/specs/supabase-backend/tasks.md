...
- [ ] 7. Update ScheduleStore with Supabase CRUD and Realtime
  - [~] 7.1 Update `src/app/context/ScheduleStore.tsx` to fetch `program_slots`, `day_attendance`, and `attendance_notifications` from Supabase on mount
  - [~] 7.2 Update `addSlot` / `removeSlot` / `assignMember` to write to `program_slots` with optimistic updates
...
- [ ] 9. Update MemberActivities page
  - [~] 9.1 Replace `mockMemberActivities` import in `src/app/pages/MemberActivities.tsx` with a `useMemberActivities` hook that fetches from `member_activities` on mount
  - [~] 9.2 Wire the "Create Activity" button to insert a new row into `member_activities`

- [ ] 10. Update TimhertAcademic page
  - [~] 10.1 Replace `useState(mockTimhertActivities)` in `src/app/pages/TimhertAcademic.tsx` with a `useTimhertActivities` hook that fetches from `timhert_activities` on mount
...
- [ ] 11. Write tests
  ...
  - [~] 11.2 Write property test (fast-check) for missing env var combinations — Feature: supabase-backend, Property 1
  - [~] 11.3 Write unit tests for AuthContext: demo mode login, live mode login, logout, onAuthStateChange, error on failure
