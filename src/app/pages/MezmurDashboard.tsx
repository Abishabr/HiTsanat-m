  ...
  useEffect(() => {
    async function load() {
      const [sdRes, sessionsRes] = await Promise.all([
        supabase.from('sub_departments').select('id').eq('name', 'Mezmur').single(),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Mezmur Sub-Department · {roleLabel}</p>
          <h1 className="text-2xl font-black text-foreground">{user?.name ?? ''}</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Choir practice and performance management</p>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[
          { label: 'Members', value: stats.memberCount, icon: Users, color: 'text-purple-600' },
          { label: 'Active Programs', value: stats.programCount, icon: Calendar, color: 'text-blue-600' },
          { label: 'Sub-Dept', value: 'Mezmur', icon: Music, color: 'text-violet-600' },
